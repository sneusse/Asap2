...
namespace Asap2
{
    internal partial class Asap2Scanner
    {
        ...
        public override void yyerror(string format, params object[] args)
        {
            ...
            errorMsg.AppendFormat("{0} : Line: {1} : Row: {2} : {3}", buffer.FileName, yyline, yycol,
        ...
        public int MakeNumber()
        {
            ...
            yylloc = new Location(yyline, yycol, yyline, yycol + yyleng, buffer.FileName);
        ...
        public int MakeHexNumber()
        {
            yylval.s = yytext;
            var tmp = yytext;
            if (tmp.StartsWith("0x", StringComparison.OrdinalIgnoreCase)) tmp = tmp.Substring(2);
        ...
        public int MakeAlignment()
        {
            ...
            try
            {
                yylval.alignment_token =
                    (ALIGNMENT.ALIGNMENT_type) Enum.Parse(typeof(ALIGNMENT.ALIGNMENT_type), yytext);
            }
            catch (ArgumentException)
            {
                throw new ParserErrorException(
                    "{0} : Line: {1} : Row: {2} : Syntax error, Unknown ALIGNMENT type: '{3}'", buffer.FileName, yyline,
                    yycol, yytext);
            }
        ...
        {
            if (fName == null)
                throw new ParserErrorException("{0} : Line: {1} : Row: {2} : Include error, /include, no filename",
                    buffer.FileName, yyline, yycol);
            ...
            try
            {
                ...
                var savedCtx = MkBuffCtx();
                if (!Path.IsPathRooted(fName))
                    /* Handle relative search path for the new file. */
                    fName = Path.Combine(Path.GetDirectoryName(buffer.FileName), fName);
                var stream = new FileStream(fName, FileMode.Open);
                SetSource(stream);
                errorHandler.reportInformation(string.Format(
                    "{0} : Line: {1} : Row: {2} : Included file \"{3}\" opened", buffer.FileName, yyline, yycol,
                    fName));
            ...
            {
                throw new ParserErrorException(
                    "{0} : Line: {1} : Row: {2} : Include error, /include, could not open file \"{3}\"",
                    buffer.FileName, yyline, yycol, fName);
            }
        }
        ...
        protected override bool yywrap()
        {
            ...
            var currentStream = streamStack.Pop();
