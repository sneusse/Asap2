...
namespace Asap2
{
    ...
    public class CHARACTERISTIC : Asap2Base, IAxisPtsCharacteristicMeasurement
    {
        ...
        [Element(10, IsList = true)] public List<ANNOTATION> annotation = new List<ANNOTATION>();

        [Element(11, IsList = true)] public List<AXIS_DESCR> axis_descr = new List<AXIS_DESCR>();

        [Element(12, IsArgument = true, CodeAsHex = true, Name = "BIT_MASK")]
        public ulong? bit_mask;

        [Element(13)] public BYTE_ORDER byte_order;

        [Element(14)] public CALIBRATION_ACCESS calibration_access;
        ...
        [Element(16)] public DEPENDENT_CHARACTERISTIC dependent_characteristic;

        [Element(17)] public DISCRETE discrete;

        [Element(18, IsArgument = true, Name = "DISPLAY_IDENTIFIER")]
        public string display_identifier;

        [Element(19)] public ECU_ADDRESS_EXTENSION ecu_address_extension;

        [Element(20)] public EXTENDED_LIMITS extended_limits;

        [Element(21, IsString = true, Name = "FORMAT")]
        public string format;

        [Element(22)] public FUNCTION_LIST function_list;

        /// <summary>
        ///     Determines that the outermost values of axes, curves and maps are calculated and cannot be adjusted.
        /// </summary>
        [Element(23)] public GUARD_RAILS guard_rails;

        [Element(34, IsList = true)] public List<IF_DATA> if_data = new List<IF_DATA>();

        [Element(24)] public MAP_LIST map_list;

        [Element(25)] public MATRIX_DIM matrix_dim;

        [Element(26)] public MAX_REFRESH max_refresh;
        ...
        /// </summary>
        [Element(27, IsArgument = true, Name = "NUMBER",
            IsObsolete = "Obsolete keyword. Please use MATRIX_DIM instead.")]
        public ulong? number;
        ...
        [Element(32)] public SYMBOL_LINK symbol_link;

        [Element(33)] public VIRTUAL_CHARACTERISTIC virtual_characteristic;

        public CHARACTERISTIC(Location location, string Name, string LongIdentifier, Type type, ulong Address,
            string Deposit, decimal MaxDiff, string Conversion, decimal LowerLimit, decimal UpperLimit) : base(location)
        {
            this.Name = Name;
        ...
        ///     the parameter ‘Conversion’ should be set to “NO_COMPU_METHOD" (measurement and calibration systems must be able to
        ///     handle this case).
        /// </summary>
        ...
        public void Validate(IErrorReporter errorReporter, MODULE module)
        {
...

            {
                ...
                if (!module.Record_layouts.ContainsKey(Deposit))
                    reportErrorOrWarning(string.Format("Referenced RECORD_LAYOUT '{0}' not found", Deposit), false,
                        errorReporter);
            }

            /* Validate that refered RECORD_LAYOUT exists */
            if (!module.Record_layouts.ContainsKey(Deposit))
                reportErrorOrWarning(string.Format("Referenced Deposit '{0}' not found", Deposit), false,
                    errorReporter);

            if (phys_unit != null && phys_unit != "")
                /* Validate that refered UNIT exists */
                if (!module.Units.ContainsKey(phys_unit))
                    reportErrorOrWarning(string.Format("Referenced UNIT '{0}' not found", phys_unit), false,
                        errorReporter);
            ...
            if (comparison_quantity != null && comparison_quantity != "")
                ...
                if (!module.AxisPtsCharacteristicMeasurement.ContainsKey(comparison_quantity))
                    reportErrorOrWarning(
                        string.Format("Referenced measurement '{0}' in COMPARISON_QUANTITY not found", Conversion),
                        false, errorReporter);

            if (Conversion != "NO_COMPU_METHOD")
                /* Validate that refered conversion method exists */
                if (!module.CompuMethods.ContainsKey(Conversion))
                    reportErrorOrWarning(string.Format("Referenced conversion method '{0}' not found", Conversion),
                        false, errorReporter);
            ...
            if (ref_memory_segment != null && ref_memory_segment != "")
            {
                ...
                if (mod_par != null)
                    if (!mod_par.memory_segment.ContainsKey(ref_memory_segment))
                        reportErrorOrWarning(
                            string.Format("Referenced MEMORY_SEGMENT '{0}' not found", ref_memory_segment), false,
                            errorReporter);
            }
        }
