...
namespace Asap2
{
    ...
    public class MODULE : Asap2Base, IValidator
    {
        ...
        [Element(1, IsArgument = true)] public string Name { get; private set; }
        ...
        {
            {
                ...
                if (list != null)
                    if (list.Count > 1)
                        list[list.Count - 1]
                            .reportErrorOrWarning("Second A2ML found, shall only be one", false, errorReporter);
            }
            {
                var list = elements.FindAll(x => x.GetType() == typeof(MOD_COMMON));
                if (list != null)
                    if (list.Count > 1)
                        list[list.Count - 1].reportErrorOrWarning("Second MOD_COMMON found, shall only be one", false,
                            errorReporter);
            }
            {
                ...
                if (list != null)
                    if (list.Count > 1)
                        list[list.Count - 1].reportErrorOrWarning("Second MOD_PAR found, shall only be one", false,
                            errorReporter);
            }
            {
                ...
                if (list != null)
                    if (list.Count > 1)
                        list[list.Count - 1].reportErrorOrWarning("Second VARIANT_CODING found, shall only be one",
                            false, errorReporter);
            }
            ...
            {
                {
                    ...
                    foreach (var item in measurements) (item as MEASUREMENT).Validate(errorReporter, this);
                }
                {
                    var measurements = elements.FindAll(x => x.GetType() == typeof(CHARACTERISTIC));
                    foreach (var item in measurements) (item as CHARACTERISTIC).Validate(errorReporter, this);
                }
                {
                    var axis_pts = elements.FindAll(x => x.GetType() == typeof(AXIS_PTS));
                    foreach (var item in axis_pts) (item as AXIS_PTS).Validate(errorReporter, this);
                }
                {
                    var compu_method = elements.FindAll(x => x.GetType() == typeof(COMPU_METHOD));
                    foreach (var item in compu_method) (item as COMPU_METHOD).Validate(errorReporter, this);
                }
            }
        }
        ...
        /// <summary>
        ///     References to the <see cref="AXIS_PTS" />s, <see cref="MEASUREMENT" />s or <see cref="CHARACTERISTIC" />s based on
        ///     the Name element.
        /// </summary>
        public Dictionary<string, IAxisPtsCharacteristicMeasurement> AxisPtsCharacteristicMeasurement =
            new Dictionary<string, IAxisPtsCharacteristicMeasurement>();
        ...
        private void AddAxisPtsCharacteristicOrMeasurement(string Name, IAxisPtsCharacteristicMeasurement obj)
        {
            ...
            catch (ArgumentException)
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'AXIS_PTS's, 'MEASUREMENT's or 'CHARACTERISTIC's in the MODULE) with name '{1}' found, ignoring",
        ...
        ///     thrown if duplicated <see cref="AXIS_PTS" />, <see cref="MEASUREMENT" /> or
        ///     <see cref="CHARACTERISTIC" /> is found.
        /// </exception>
        ...
        /// <summary>
        ///     References to the <see cref="COMPU_TAB" />s, <see cref="COMPU_VTAB" />s or <see cref="COMPU_VTAB_RANGE" />s based
        ///     on the Name element.
        /// </summary>
        ...
        private void AddCompuTabCompuVtabCompuVtabRanges(string Name, Asap2Base obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'COMPU_TABs', 'COMPU_VTAB's or'COMPU_VTAB_RANGE's in the MODULE) with name '{1}' found, ignoring",
                    obj.GetType().Name, Name));
            }
        }
        ...
        /// <exception cref="ValidationErrorException">
        ///     thrown if duplicated <see cref="COMPU_TAB" />, <see cref="COMPU_VTAB" /> or
        ///     <see cref="COMPU_VTAB_RANGE" /> is found.
        /// </exception>
        ...
        public void AddElement(COMPU_METHOD obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'COMPU_METHODs' in the MODULE) with name '{1}' found, ignoring",
        ...
        public void AddElement(FRAME obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'FRAMEs' in the MODULE) with name '{1}' found, ignoring",
                    obj.GetType().Name, obj.Name));
            }
        }
        ...
        public void AddElement(FUNCTION obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'FUNCTIONs' in the MODULE) with name '{1}' found, ignoring",
                    obj.GetType().Name, obj.Name));
            }
        }
        ...
        public void AddElement(RECORD_LAYOUT obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'RECORD_LAYOUTs' in the MODULE) with name '{1}' found, ignoring",
                    obj.GetType().Name, obj.Name));
            }
        }
        ...
        public void AddElement(UNIT obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'UNITs' in the MODULE) with name '{1}' found, ignoring",
                    obj.GetType().Name, obj.Name));
            }
        }
        ...
        public void AddElement(USER_RIGHTS obj)
        {
            ...
            {
                throw new ValidationErrorException(string.Format(
                    "Warning: Duplicate '{0}' (Must be unique of all 'USER_RIGHTSs' in the MODULE) with name '{1}' found, ignoring",
                    obj.GetType().Name, obj.Name));
            }
        }
