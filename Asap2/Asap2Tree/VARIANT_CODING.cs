...
namespace Asap2
{
    ...
    ///     The description of variant-coded parameters and non-variant coded parameters does not differ in the
    ///     <see cref="CHARACTERISTIC" /> keyword.
    ///     If a parameter shall be variant coded, then <see cref="VARIANT_CODING" /> has a reference to this parameter and
    ///     specifies additional properties
    ///     that describe how to select the active variant (i.e. read the selected value from memory).
    ///     The active variant might be selected by another tunable parameter (<see cref="VAR_SELECTION_CHARACTERISTIC" />) or
    ///     an ECU-internal variable (<see cref="VAR_MEASUREMENT" />).
    /// </summary>
    [Base]
    public class VARIANT_CODING : Asap2Base
    {
        ...
        [Element(1, IsList = true)] public List<VAR_CHARACTERISTIC> var_characteristic = new List<VAR_CHARACTERISTIC>();

        [Element(2, IsList = true)] public List<VAR_CRITERION> var_criterion = new List<VAR_CRITERION>();
