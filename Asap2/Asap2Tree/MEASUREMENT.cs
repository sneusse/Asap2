...
namespace Asap2
{
    ...
    public class MEASUREMENT : Asap2Base, IAxisPtsCharacteristicMeasurement
    {
        ...
        [Element(9, IsList = true)] public List<ANNOTATION> annotation = new List<ANNOTATION>();

        [Element(10)] public ARRAY_SIZE array_size;

        [Element(11, IsArgument = true, CodeAsHex = true, Name = "BIT_MASK")]
        public ulong? bit_mask;

        [Element(12)] public BIT_OPERATION bit_operation;

        [Element(13)] public BYTE_ORDER byte_order;

        [Element(14)] public DISCRETE discrete;

        [Element(15, IsArgument = true, Name = "DISPLAY_IDENTIFIER")]
        public string display_identifier;

        [Element(16)] public ECU_ADDRESS ecu_address;

        [Element(17)] public ECU_ADDRESS_EXTENSION ecu_address_extension;
        ...
        [Element(20)] public FUNCTION_LIST function_list;

        [Element(30, IsList = true)] public List<IF_DATA> if_data = new List<IF_DATA>();

        [Element(21, IsArgument = true, Name = "LAYOUT")]
        public LAYOUT? layout;

        [Element(22)] public MATRIX_DIM matrix_dim;

        [Element(23)] public MAX_REFRESH max_refresh;
        ...
        [Element(27)] public SYMBOL_LINK symbol_link;

        [Element(28)] public VIRTUAL Virtual;

        public MEASUREMENT(Location location, string Name, string LongIdentifier, DataType Datatype, string Conversion,
            uint Resolution, decimal Accuracy, decimal LowerLimit, decimal UpperLimit) : base(location)
        {
            this.Name = Name;
        ...
        {
            ulong Address = 0;
            if (ecu_address != null) Address = ecu_address.value;
            return Address;
        }
        ...
        {
            if (ecu_address == null)
                ecu_address = new ECU_ADDRESS(location, address);
            else
