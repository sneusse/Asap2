...
namespace QUT.Gppg
{
        ...
        where TSpan : IMerge<TSpan>, new()
    {
                ...
                if (action > 0) // shift
                    ...
                    catch (Exception x)
                    {
                        if (x is AbortException)
                            return false;
                        if (x is AcceptException)
                            return true;
                        if (x is ErrorException && !ErrorRecovery())
                            return false;
        ...
        private bool DiscardInvalidTokens()
        {
            ...
            if (FsaState.ParserTable != null) // Discard tokens until find one that works ...
                while (true)
                {
                    if (NextToken == 0)
                    {
