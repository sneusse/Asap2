...
namespace Asap2.Tests
{
    ...
    public class ParserTests
    {
        ...
        public void ParseA2LFiles(string fileName)
        {
            ...
            var parser = new Parser(fullPath, errorReporter);
            var tree = parser.DoParse();

            Assert.That(errorReporter.Errors.Count, Is.Zero,
                () => "Had errors: " + string.Join("\r\n - ", errorReporter.Errors));
            Assert.That(errorReporter.Warnings.Count, Is.Zero,
                () => "Had warnings: " + string.Join("\r\n - ", errorReporter.Warnings));
